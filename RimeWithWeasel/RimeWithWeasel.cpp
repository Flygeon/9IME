...
static void _UpdateUIStyle(RimeConfig* config, UI* ui, bool initialize) {
  ...
  // 9IME: optional Sogou skin (.ssf) file. A relative path is resolved
  // against the Rime user data directory (e.g. %APPDATA%\Rime).
  {
    ...
    if (rime_api->config_get_string(config, "style/skin", skin_buf, SKIN_BUF)) {
      ...
      else if (skin_path.find(L":") != std::wstring::npos ||
               (!skin_path.empty() && skin_path[0] == L'\\'))
