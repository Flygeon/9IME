...
void WeaselPanel::_ResizeWindow() {
  ...
  if (m_skin.Loaded()) {
    const bool vertical = (m_style.layout_type == UIStyle::LAYOUT_VERTICAL);
    const SogouSkin::Scheme& sc =
        vertical ? m_skin.vertical() : m_skin.horizontal();
    if (m_show_tips && m_skin.status_bar_pic()) {
      // floating Sogou status bar
      m_size.cx = DPI_SCALE(m_skin.status_bar_w());
      m_size.cy = DPI_SCALE(m_skin.status_bar_h());
    } else if (sc.pic) {
      // engine content size includes the symmetric margins; the skin
      // stretches the background image around the text block
      int cw = m_size.cx - 2 * DPI_SCALE(m_style.margin_x);
      int ch = m_size.cy - 2 * DPI_SCALE(m_style.margin_y);
      if (cw < 0)
        cw = 0;
      if (ch < 0)
        ch = 0;
      int text_right = min(sc.preedit_right, sc.candidate_right);
      int w = DPI_SCALE(sc.preedit_left) + cw +
              (DPI_SCALE(sc.img_w) - DPI_SCALE(text_right));
      int h = DPI_SCALE(sc.preedit_top) + ch +
              (DPI_SCALE(sc.img_h) - DPI_SCALE(sc.candidate_bottom));
      m_size.cx = max(DPI_SCALE(sc.img_w), w);
      m_size.cy = max(DPI_SCALE(sc.img_h), h);
    }
  }
...
// 9IME: load the Sogou skin configured via style/skin and apply its
// parameters onto the current style. Idempotent; called on every refresh so
// schema switches or config reloads are picked up automatically.
void WeaselPanel::_EnsureSkin() {
  const bool want_skin =
      !m_style.skin_file.empty() && !IS_FULLSCREENLAYOUT(m_style) &&
      m_style.layout_type != UIStyle::LAYOUT_VERTICAL_TEXT;
  if (want_skin) {
    // refresh the monitor dpi so skin values scale correctly
    HMONITOR hm = MonitorFromRect(m_inputPos, MONITOR_DEFAULTTONEAREST);
    UINT dpiX = 96, dpiY = 96;
    if (hm)
    ...
    if (m_skin.Loaded())
      _ApplySkinStyle();
  } else if (m_skin.Loaded()) {
    m_skin.Unload();
    m_skin_file.clear();
  }
}

// map the Sogou skin onto the Weasel style used by the layout engines
void WeaselPanel::_ApplySkinStyle() {
  const bool vertical = (m_style.layout_type == UIStyle::LAYOUT_VERTICAL);
  const SogouSkin::Scheme& sc =
      vertical ? m_skin.vertical() : m_skin.horizontal();
  if (!sc.pic)
    return;  // this orientation has no scheme; keep the built-in style

  // font: skin font size is in pixels at 96 dpi; Weasel uses points
  const int font_px = m_skin.font_size();
  int font_pt = MulDiv(font_px, 72, 96);
  if (font_pt <= 0)
    font_pt = 12;
  if (!m_skin.font_name().empty())
    m_style.font_face = m_skin.font_name() + L",Microsoft YaHei,SimSun";
  m_style.font_point = font_pt;
  m_style.label_font_point = font_pt;
  m_style.comment_font_point = font_pt;

  // colors (skin colors already converted to COLORREF)
  m_style.text_color = 0xff000000 | m_skin.preedit_color();
  m_style.hilited_text_color = 0xff000000 | m_skin.hilited_candidate_color();
  m_style.candidate_text_color = 0xff000000 | m_skin.candidate_color();
  m_style.hilited_candidate_text_color =
      0xff000000 | m_skin.hilited_candidate_color();
  m_style.label_text_color = m_style.candidate_text_color;
  m_style.hilited_label_text_color = m_style.hilited_candidate_text_color;
  m_style.comment_text_color = m_style.candidate_text_color;
  m_style.hilited_comment_text_color = m_style.hilited_candidate_text_color;
  m_style.back_color = 0xff000000 | m_skin.fallback_back_color();
  // the skin images replace the plain color backgrounds
  m_style.candidate_back_color = 0;
  m_style.hilited_candidate_back_color = 0;
  m_style.candidate_border_color = 0;
  m_style.hilited_candidate_border_color = 0;
  m_style.border_color = 0;
  m_style.shadow_color = 0;
  m_style.hilited_candidate_shadow_color = 0;
  m_style.prevpage_color = 0;
  m_style.nextpage_color = 0;
  m_style.hilited_mark_color = 0;
  m_style.border = 0;
  m_style.shadow_radius = 0;
  m_style.shadow_offset_x = 0;
  m_style.shadow_offset_y = 0;
  m_style.round_corner = 0;
  m_style.round_corner_ex = 0;
  m_style.inline_preedit = false;
  m_style.max_width = 0;
  m_style.max_height = 0;

  // geometry: place the text block exactly per the skin areas
  m_style.margin_x = max(sc.preedit_left, sc.candidate_left);
  m_style.margin_y = sc.preedit_top;
  // the preedit line is taller than the font size; compensate so the
  // candidates row lands on the skin's candidate area
  const int line_h = MulDiv(font_px, 4, 3);
  m_style.spacing = max(0, sc.gap - (line_h - font_px));
  m_style.candidate_spacing = MulDiv(font_px, 1, 6);
  m_style.hilite_spacing = MulDiv(font_px, 1, 4);
  m_style.hilite_padding_x = 0;
  m_style.hilite_padding_y = 0;
}

const SogouSkin::Scheme* WeaselPanel::_SkinScheme() const {
  if (!m_skin.Loaded())
    return NULL;
  const bool vertical = (m_style.layout_type == UIStyle::LAYOUT_VERTICAL);
  const SogouSkin::Scheme& sc =
      vertical ? m_skin.vertical() : m_skin.horizontal();
  return sc.pic ? &sc : NULL;
}
  ...
  if (!t.empty()) {
    ...
    if (range.start < range.end) {
      ...
      }
      {
        ...
        // 9IME: draw the skin's preedit highlight image
        {
          const SogouSkin::Scheme* sc = _SkinScheme();
          if (sc && sc->preedit_highlight)
            m_skin.DrawHighlight(dc, rc_hi, sc->preedit_highlight);
        }
      }
    }
    drawn = true;
...
bool WeaselPanel::_DrawCandidates(CDCHandle& dc, bool back) {
  ...
  if (back) {
    ...
    // draw highlighted background and shadow
    {
      ...
      // 9IME: draw the skin's highlighted-candidate image
      {
        const SogouSkin::Scheme* sc = _SkinScheme();
        if (sc && sc->candidate_highlight)
          m_skin.DrawHighlight(dc, rect, sc->candidate_highlight);
      }
...
void WeaselPanel::DoPaint(CDCHandle dc) {
  ...
  ReleaseDC(hdc);
  bool drawn = false;
  const bool skin_vertical =
      (m_style.layout_type == UIStyle::LAYOUT_VERTICAL);
  if (m_skin.Loaded() && m_show_tips && m_skin.status_bar_pic()) {
    // 9IME: floating Sogou status bar (bar.png + state icons)
    m_skin.DrawStatusBar(memDC, rcw, m_status.ascii_mode,
                         m_status.full_shape, m_status.disabled);
    drawn = true;
  } else if (!hide_candidates) {
    ...
    if ((!m_ctx.empty() && !m_style.inline_preedit) ||
      ...
      if (m_skin.Loaded() && m_skin.HasScheme(skin_vertical)) {
    ...
    if (m_layout->ShouldDisplayStatusIcon()) {
      ...
      if (m_skin.Loaded()) {
        // 9IME: draw the skin's own cn/en state icon
        m_skin.DrawCnEnIcon(memDC, iconRect, m_status.ascii_mode,
                            m_status.disabled);
        drawn = true;
      } else {
