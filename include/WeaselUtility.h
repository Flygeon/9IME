...
inline fs::path WeaselLogPath() {
  ...
  // default location
  ExpandEnvironmentStringsW(L"%TEMP%\\rime.9ime", _path, _countof(_path));
...
inline std::wstring get_weasel_ime_name() {
  ...
  // 9IME: the input method is branded "9IME" in every locale
  (void)langId;
  return L"9IME";
}
...
inline LANGID get_language_id() {
  std::wstring lang{};
  if (RegGetStringValue(HKEY_CURRENT_USER, L"Software\\Rime\\9IME",
