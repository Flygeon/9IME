    ...
    if ($os -eq "Windows" -and $build_variant -eq "msvc") {
      ...
      if ( $use -eq "dev"){
        ...
        if ([Environment]::Is64BitOperatingSystem) {
          $registryPath = "HKLM:\SOFTWARE\WOW6432Node\Rime\9IME"
        } else {
          $registryPath = "HKLM:\SOFTWARE\Rime\9IME"
        }
        try {
          $weaselRoot = (Get-ItemProperty -Path $registryPath -ErrorAction Stop).'9IMERoot'
