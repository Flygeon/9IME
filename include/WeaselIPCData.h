...
struct UIStyle {
  ...
  // 9IME: Sogou skin (.ssf) file to render with, empty to use the built-in
  // color scheme. Server-side only; intentionally not serialized over IPC.
  ...
  bool operator!=(const UIStyle& st) {
    return (
        ...
        skin_file != st.skin_file || hover_type != st.hover_type ||
