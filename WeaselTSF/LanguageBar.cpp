...
static LPCWSTR GetWeaselRegName() {
  ...
  if (is_wow64())
    WEASEL_REG_NAME_ = L"Software\\WOW6432Node\\Rime\\9IME";
  else
    WEASEL_REG_NAME_ = L"Software\\Rime\\9IME";
...
void WeaselTSF::_HandleLangBarMenuSelect(UINT wID) {
  ...
  switch (wID) {
    ...
    case ID_WEASELTRAY_USERCONFIG:
      if (FAILED(RegGetStringValue(HKEY_CURRENT_USER, L"Software\\Rime\\9IME",
                                   L"RimeUserDir", dir)) ||
          dir.empty()) {
